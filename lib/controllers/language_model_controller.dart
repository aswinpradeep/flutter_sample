...
class LanguageModelController extends GetxController {
          ...
          : () {
              try {
                ...
                for (String eachUseableLangPair in canUseSourceAndTargetLangSet) {
                  _allAvailableSourceLanguages
                      .add(AppConstants.getLanguageCodeOrName(value: eachUseableLangPair.split('-')[0], returnWhat: LANGUAGE_MAP.languageName));
                  _allAvailableTargetLanguages
                      .add(AppConstants.getLanguageCodeOrName(value: eachUseableLangPair.split('-')[1], returnWhat: LANGUAGE_MAP.languageName));
                }
  ...
  void changeSelectedSourceLangAndCalcTargetLangs({required String selectedSourceLanguageName}) {
    ...
    _appUIController.changeCurrentRequestStatusForUI(newStatus: AppConstants.INITIAL_CURRENT_STATUS_VALUE);
    ...
    for (var eachAvailableModel in availableTranslationModelsList) {
      if (eachAvailableModel.languages[0].sourceLanguage ==
          AppConstants.getLanguageCodeOrName(value: selectedSourceLanguageName, returnWhat: LANGUAGE_MAP.languageCode)) {
        availableTranslationModelsTargetLangForSelectedSourceLang.add(AppConstants.getLanguageCodeOrName(
            value: eachAvailableModel.languages[0].targetLanguage.toString(), returnWhat: LANGUAGE_MAP.languageName));
      }
    }
